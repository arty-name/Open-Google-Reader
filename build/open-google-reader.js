...
function ui() {
  ...
  // tags/folders will be here
  var tagList;
  
  // will hold current Tag/Folder
  var currentLabel;
...
function updateTags()
{
tSelector = document.getElementById('tagSelector');


while (tSelector.firstChild) {
 tSelector.removeChild(tSelector.firstChild);
}
...
for (cTag in tagList)
{
//tList[tList.length] = DOM('option', {value: 'test', innerHTML: 'test' });
tagId = tagList[cTag]['id'];
tagName = tagList[cTag]['id'];
tagName = tagName.replace(/user\/.*\/label\//,"");
if (tagId.match(/\/label\//)) 
tSelector.appendChild(DOM('option', {value: tagId, innerHTML: tagName }));
}

}

function tagSelectorChanged(event)
{
currentLabel = event.target.value;
tags = initTags();
actions.reload();
//alert('selector changed ' + currentLabel);
}
...
if (currentLabel) unreadValue = currentLabel;
else unreadValue = tagPrefix + 'reading-list';

    // various "tags": some to filter entries, some to assign
    return {
//     unread: tagPrefix + 'reading-list',
// put labelname here like /label/TG
//     unread: 'user/' + userId + '/label/TG',
unread: unreadValue,
      read: tagPrefix + 'read',
      like: tagPrefix + 'like',
...
//tList = [DOM('option', {value: 'loading', innerHTML: 'loading' })];
tList = [];

dropDown =  DOM('select',{id: 'tagSelector'}, tList );
dropDown.addEventListener('change', tagSelectorChanged, false);
//onChange: tagSelectorChanged()
return dropDown;
}
...
// put label name here like /\/label\/labelname/
//  if (feed.id.match(/\/label\/T/)) count += feed.count;
if (currentLabel) 
{
if (feed.id == currentLabel) count += feed.count;
}
else
{
 if (feed.id.match(/^feed/)) count += feed.count;
...
tSelector = document.getElementById('tagSelector');
 ...
 var children = tSelector.childNodes;

     // Loop through the children
     for(var c=0; c < children.length; c++) {

if (children[c].value == feed.id) children[c].innerHTML = children[c].innerHTML + ' (' + feed.count +')';
     }
...
// will load Tags/folders here
function loadTags()
{

  return APIRequest('tag/list', {
      parameters: {output: 'json'},
      onSuccess: function(response) {
       tagList = response.responseJSON.tags;
//for (tag in tagList[0])
//{
//alert ("tag0 field " + cfield + " is " + tagList[0][cfield]);
//}
updateTags();
        }
    });
  ...
  function keyHandler(event){ try{
    ...
    switch (key) {
      ...
      case 'C': case 'С': actions.next(); break;
    ...
    matched = true;
    switch (key) {
      case 'Z': case 'Я': actions.prev(); break;
      case 'V': case 'М': currentEntry && openTab(currentEntry.querySelector('a'), event); break;
      case 'J': case 'О': currentEntry && openTab(currentEntry.querySelectorAll('a')[1], event); break;
      case 'X': case 'Ч': actions[event.shiftKey ? 'share' : 'star'](); break;
