...
function ui() {
  ...
  function getNewUnreadContinuation() {
    ...
    // otherwise getting new continuation is a bit more complex
    } else {
      ...
      AjaxRequest(entriesUrl + tags['unread'], {
        parameters: {n: displayed, xt: tags.read, r: sort.oldestFirst},
        onSuccess: function(response){
          var data = response.responseJSON;
          if (!data) return;
  ...
  actions = {
    ...
    removeReadUnstarred: function() {
      ...
      if (!currentEntry.parentNode) currentEntry = container.querySelector('section.entry');
